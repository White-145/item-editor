...
public class BookNode implements Node {
    private static final CommandSyntaxException ISNT_WRITTEN_BOOK_EXCEPTION = new SimpleCommandExceptionType(Component.translatable("commands.edit.book.error.isntwrittenbook")).create();
    private static final CommandSyntaxException ISNT_BOOK_EXCEPTION = new SimpleCommandExceptionType(Component.translatable("commands.edit.book.error.isntbook")).create();
    private static final CommandSyntaxException NO_TITLE_EXCEPTION = new SimpleCommandExceptionType(Component.translatable("commands.edit.book.error.notitle")).create();
    private static final CommandSyntaxException NO_AUTHOR_EXCEPTION = new SimpleCommandExceptionType(Component.translatable("commands.edit.book.error.noauthor")).create();
    private static final CommandSyntaxException NO_PAGES_EXCEPTION = new SimpleCommandExceptionType(Component.translatable("commands.edit.book.error.nopages")).create();
    private static final CommandSyntaxException TITLE_ALREADY_IS_EXCEPTION = new SimpleCommandExceptionType(Component.translatable("commands.edit.book.error.titlealreadyis")).create();
    private static final CommandSyntaxException AUTHOR_ALREADY_IS_EXCEPTION = new SimpleCommandExceptionType(Component.translatable("commands.edit.book.error.authoralreadyis")).create();
    private static final CommandSyntaxException GENERATION_ALREADY_IS_EXCEPTION = new SimpleCommandExceptionType(Component.translatable("commands.edit.book.error.generationalreadyis")).create();
    private static final CommandSyntaxException PAGE_ALREADY_IS_EXCEPTION = new SimpleCommandExceptionType(Component.translatable("commands.edit.book.error.pagealreadyis")).create();
    ...
    private static boolean hasPages(ItemStack stack) {
        if (stack.getItem() == Items.WRITABLE_BOOK) {
            ...
            return !stack.get(DataComponents.WRITABLE_BOOK_CONTENT).pages().isEmpty();
        }
        ...
        return !stack.get(DataComponents.WRITTEN_BOOK_CONTENT).pages().isEmpty();
    }
    ...
    private static String getTitle(ItemStack stack) {
        ...
        return stack.get(DataComponents.WRITTEN_BOOK_CONTENT).title().get(false);
    }
    ...
    private static Generation getGeneration(ItemStack stack) {
        ...
        Generation generation = Generation.byGeneration(stack.get(DataComponents.WRITTEN_BOOK_CONTENT).generation());
        return generation == null ? Generation.ORIGINAL : generation;
    }

    private static List<Component> getPages(ItemStack stack) {
        if (stack.getItem() == Items.WRITABLE_BOOK) {
            if (!stack.has(DataComponents.WRITABLE_BOOK_CONTENT)) {
                return new ArrayList<>();
            }
            return new ArrayList<>(stack.get(DataComponents.WRITABLE_BOOK_CONTENT).pages().stream().map(page -> (Component)Component.literal(page.get(false))).toList());
        }
        if (!stack.has(DataComponents.WRITTEN_BOOK_CONTENT)) {
            return new ArrayList<>();
        }
        return new ArrayList<>(stack.get(DataComponents.WRITTEN_BOOK_CONTENT).pages().stream().map(page -> page.get(false)).toList());
    }
    ...
    private static void setTitle(ItemStack stack, String title) {
        ...
        if (stack.has(DataComponents.WRITTEN_BOOK_CONTENT)) {
            WrittenBookContent component = stack.get(DataComponents.WRITTEN_BOOK_CONTENT);
            pages = component.pages();
            author = component.author();
            generation = component.generation();
        }
        stack.set(DataComponents.WRITTEN_BOOK_CONTENT, new WrittenBookContent(Filterable.passThrough(title), author, generation, pages, false));
    }

    private static void setAuthor(ItemStack stack, String author) {
        List<Filterable<Component>> pages = List.of();
        Filterable<String> title = Filterable.from(FilteredText.passThrough(DEFAULT_TITLE));
        int generation = Generation.ORIGINAL.generation;
        if (stack.has(DataComponents.WRITTEN_BOOK_CONTENT)) {
            WrittenBookContent component = stack.get(DataComponents.WRITTEN_BOOK_CONTENT);
            pages = component.pages();
        ...
        stack.set(DataComponents.WRITTEN_BOOK_CONTENT, new WrittenBookContent(title, author, generation, pages, false));
    }

    private static void setGeneration(ItemStack stack, Generation generation) {
        List<Filterable<Component>> pages = List.of();
        String author = DEFAULT_AUTHOR;
        Filterable<String> title = Filterable.passThrough(DEFAULT_TITLE);
        if (stack.has(DataComponents.WRITTEN_BOOK_CONTENT)) {
            WrittenBookContent component = stack.get(DataComponents.WRITTEN_BOOK_CONTENT);
            pages = component.pages();
            author = component.author();
        ...
        stack.set(DataComponents.WRITTEN_BOOK_CONTENT, new WrittenBookContent(title, author, generation.generation, pages, false));
    }

    private static void setPages(ItemStack stack, List<Component> pages) {
        if (stack.getItem() == Items.WRITABLE_BOOK) {
            stack.set(DataComponents.WRITABLE_BOOK_CONTENT, new WritableBookContent(pages.stream().map(EditorUtil::textToString).map(Filterable::passThrough).toList()));
        } else {
            Filterable<String> title = Filterable.passThrough(DEFAULT_TITLE);
            String author = DEFAULT_AUTHOR;
            int generation = 0;
            ...
            stack.set(DataComponents.WRITTEN_BOOK_CONTENT, new WrittenBookContent(title, author, generation, pages.stream().map(Filterable::passThrough).toList(), false));
        }
    }
    ...
    public <S extends SharedSuggestionProvider> CommandNode<S> register(CommonCommandManager<S> commandManager, CommandBuildContext registryAccess) {
        ...
        CommandNode<S> titleGetNode = commandManager.literal("get").executes(context -> {
            ...
            EditorUtil.sendFeedback(context.getSource(), Component.translatable(OUTPUT_GET_TITLE, title));
        ...
        CommandNode<S> titleSetTitleNode = commandManager.argument("title", StringArgumentType.greedyString()).executes(context -> {
            ...
            EditorUtil.sendFeedback(context.getSource(), Component.translatable(OUTPUT_SET_TITLE, title));
            return Command.SINGLE_SUCCESS;
        }).build();
        ...
        CommandNode<S> authorGetNode = commandManager.literal("get").executes(context -> {
            ...
            EditorUtil.sendFeedback(context.getSource(), Component.translatable(OUTPUT_GET_AUTHOR, author));
            return Command.SINGLE_SUCCESS;
        }).build();
        ...
        CommandNode<S> authorSetAuthorNode = commandManager.argument("author", StringArgumentType.greedyString()).executes(context -> {
            ...
            EditorUtil.setStack(context.getSource(), stack);
            EditorUtil.sendFeedback(context.getSource(), Component.translatable(OUTPUT_SET_AUTHOR, author));
            return Command.SINGLE_SUCCESS;
        }).build();
        ...
        CommandNode<S> generationGetNode = commandManager.literal("get").executes(context -> {
            ...
            EditorUtil.sendFeedback(context.getSource(), Component.translatable(OUTPUT_GET_GENERATION, Component.translatable(generation.translationKey)));
            return Command.SINGLE_SUCCESS;
        }).build();
        ...
        CommandNode<S> generationSetGenerationNode = commandManager.argument("generation", EnumArgumentType.enums(Generation.class)).executes(context -> {
            ...
            EditorUtil.setStack(context.getSource(), stack);
            EditorUtil.sendFeedback(context.getSource(), Component.translatable(OUTPUT_SET_GENERATION, Component.translatable(generation.translationKey)));
            return Command.SINGLE_SUCCESS;
        }).build();
        ...
        CommandNode<S> pageGetNode = commandManager.literal("get").executes(context -> {
            ...
            List<Component> pages = getPages(stack);

            EditorUtil.sendFeedback(context.getSource(), Component.translatable(OUTPUT_PAGE_GET));
            for (int i = 0; i < pages.size(); ++i) {
                EditorUtil.sendFeedback(context.getSource(), Component.empty().append(Component.literal(i + ". ").setStyle(Style.EMPTY.withColor(ChatFormatting.GRAY))).append(TextUtil.copyable(pages.get(i))));
            }
            return Command.SINGLE_SUCCESS;
        }).build();
        ...
        CommandNode<S> pageGetIndexNode = commandManager.argument("index", IntegerArgumentType.integer(0, 99)).executes(context -> {
            ...
            EditorUtil.sendFeedback(context.getSource(), Component.translatable(OUTPUT_PAGE_GET_SINGLE, TextUtil.copyable(page)));
            return Command.SINGLE_SUCCESS;
        }).build();
        ...
        CommandNode<S> pageSetIndexNode = commandManager.argument("index", IntegerArgumentType.integer(0, 99)).executes(context -> {
            ...
            if (index >= pages.size()) {
                ...
                Component oldPage = pages.get(index);
            ...
            if (!isWritten(stack)) {
                page = Component.literal(EditorUtil.textToString(page));
            }

            EditorUtil.setStack(context.getSource(), stack);
            EditorUtil.sendFeedback(context.getSource(), Component.translatable(OUTPUT_PAGE_SET, TextUtil.copyable(page)));
            return Command.SINGLE_SUCCESS;
        }).build();
        ...
        CommandNode<S> pageSetIndexPageNode = commandManager.argument("page", LegacyTextArgumentType.text()).executes(context -> {
            ...
            int index = IntegerArgumentType.getInteger(context, "index");
            Component page = LegacyTextArgumentType.getText(context, "page");
            List<Component> pages = getPages(stack);
            if (index >= pages.size()) {
                int off = index - pages.size() + 1;
                for (int i = 0; i < off; ++i) {
        ...
        CommandNode<S> pageRemoveIndexNode = commandManager.argument("index", IntegerArgumentType.integer(0)).executes(context -> {
            ...
            EditorUtil.setStack(context.getSource(), stack);
            EditorUtil.sendFeedback(context.getSource(), Component.translatable(OUTPUT_PAGE_REMOVE));
            return Command.SINGLE_SUCCESS;
        }).build();
        ...
        CommandNode<S> pageAddNode = commandManager.literal("add").executes(context -> {
            ...
            EditorUtil.setStack(context.getSource(), stack);
            EditorUtil.sendFeedback(context.getSource(), Component.translatable(OUTPUT_PAGE_ADD, TextUtil.copyable(page)));
            return Command.SINGLE_SUCCESS;
        }).build();
        ...
        CommandNode<S> pageInsertIndexNode = commandManager.argument("index", IntegerArgumentType.integer(0, 99)).executes(context -> {
            ...
            EditorUtil.setStack(context.getSource(), stack);
            EditorUtil.sendFeedback(context.getSource(), Component.translatable(OUTPUT_PAGE_INSERT, TextUtil.copyable(page)));
            return Command.SINGLE_SUCCESS;
        }).build();
        ...
        CommandNode<S> pageClearNode = commandManager.literal("clear").executes(context -> {
            ...
            EditorUtil.setStack(context.getSource(), stack);
            EditorUtil.sendFeedback(context.getSource(), Component.translatable(OUTPUT_PAGE_CLEAR));
            return Command.SINGLE_SUCCESS;
        }).build();
        ...
        CommandNode<S> pageClearBeforeIndexNode = commandManager.argument("index", IntegerArgumentType.integer(1)).executes(context -> {
            ...
            EditorUtil.setStack(context.getSource(), stack);
            EditorUtil.sendFeedback(context.getSource(), Component.translatable(OUTPUT_PAGE_CLEAR_BEFORE, index));
            return Command.SINGLE_SUCCESS;
        }).build();
