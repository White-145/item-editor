...
public class DurationArgumentType implements ArgumentType<Integer> {
    ...
    private static final SimpleCommandExceptionType INVALID_UNIT_EXCEPTION = new SimpleCommandExceptionType(Component.translatable("argument.duration.error.invalidunit"));
    private static final Dynamic2CommandExceptionType DURATION_TOO_LOW_EXCEPTION = new Dynamic2CommandExceptionType((value, minimum) -> Component.translatable("argument.duration.error.toolow", minimum, value));
    ...
    public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> context, SuggestionsBuilder builder) {
        ...
        if (stringReader.readUnquotedString().isEmpty()) {
            for (Unit unit : Unit.values()) {
                builder.suggest(unit.suffix, Component.translatable(unit.suggestion));
            }
        }
        return builder.buildFuture();
    }
